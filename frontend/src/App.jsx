...
function App() {
  ...
  async function fetchQuestionsAndAnswers() {
    try {
      ...
      setLoadingQ(true);
      setLoadingA(true);
      const questionsResponse = await axios.post('/api/generate-questions', { text: inputText });
      setQuestions(questionsResponse.data);
      setLoadingQ(false);

      const answersResponse = await axios.get('/api/generate-answers');
  ...
  return (
    ...
    <h3>Enter text to generate questions</h3>
    <textarea value={inputText} onChange={(e) => setInputText(e.target.value)}></textarea>
    <button onClick={fetchQuestionsAndAnswers}>Generate Questions</button>
    {visible?(<div>
